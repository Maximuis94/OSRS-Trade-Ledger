...
TODO graph model / controller
    Current methods for plotting graphs have lots of room for improvement, in particular w.r.t. OO-design.
    1. Design base classes involved in plotting
        -> Evaluate current implementation
        -> Apply+combine new insights, e.g. multiple inheritance, namedtuples, etc.
    2. Restructure existing graph code (MVC pattern / graph util / global vars)
    3. Implement controllers for plotting specific graphs, using the base class as template
        -> Use (improved versions of) current implementations if possible
"""
...
def price_graph_by_dow(axs: plt.Axes, item: NpyArray, n_weeks: int = 4, colors: collections.abc.Mapping = None,
    ...
    t1 = int(time.time())
    t1 = t1 - t1 % ts_cutoff
    t0 = ts_to_dt(t1 - t1 % 604800-604800*n_weeks) - datetime.timedelta(days=datetime.datetime.utcfromtimestamp(time.time()).weekday()+1)
    ct = int(time.time())
    if colors is None:
    ...
    n_cutoff *= 2
    volatility = (np.average(avg_prices)-int(np.average(avg_prices)*.01)) * (y_merged[-n_cutoff]-y_merged[n_cutoff]) * \
    ...
    axs.vlines(x=[day_id*86400 for day_id in range(7)], ymin=y0, ymax=y1, colors=(0., 0., 0., 1.), label='Current timestamp', linewidths=1)
...
def price_graph_by_hod(axs: plt.Axes, item: NpyArray, n_days: int = 7, colors: collections.abc.Mapping = None,
    ...
    if colors is None:
        colors = [((20+i*30)%255/255, (100+30*i//3)%255/255, (80+30*i//4)%255/255, 1.) for i in range(n_days+1)]
        # colors = ['b', 'g', 'c', 'm', 'y', 'k']
