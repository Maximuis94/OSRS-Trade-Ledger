...
class ItemDB(Database):
    ...
    def __init__(self, augment_items: bool = True, read_only: bool = False, **kwargs):
        ...
        super().__init__(File(path), read_only=read_only, tables=self.table_name,
                         row_tuple=namedtuple("ItemTuple", Item.sqlite_columns()))
    ...
    def get_item(self, item_id: int, augment_items: bool = None) -> Item:
        ...
        try:
            return self.execute(self.select_by_item_id, {'item_id': item_id}).fetchone()
        except OSError as e:
            print(item_id, self.select_by_item_id)
            raise e
    ...
    def get_item(self, item_name: str, augment_items: bool = None) -> Item:
        ...
        self.set_item_factory(augment_items)
        try:
            return self.execute(self.select_by_item_name, {'item_name': item_name}).fetchone()
        except OSError as e:
            print(item_name, self.select_by_item_name)
            raise e
    
    def all_items(self, augment_items: bool = None) -> Tuple[Item, ...]:
        """ Load all item rows from the database and return them """
        # print(self.table.select)
        self.set_item_factory(augment_items)
        return tuple(self.execute(self.table.select).fetchall())
    ...
    @property
    def select_by_item_id(self) -> str:
        """Return a select statement that searches by item_id"""
        return self.table.select + "WHERE item_id=:item_id"
    
    @property
    def select_by_item_name(self) -> str:
        """Return a select statement that searches by item_name"""
        return self.table.select + "WHERE item_name=:item_name"
