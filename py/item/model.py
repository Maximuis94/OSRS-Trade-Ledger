"""Module with the Item model implementation"""
import sqlite3
...
class Item:
    """
    Model class of an OSRS Item.
    
    Representation of an OSRS Item. An Item has various types of variables;
    - Static: Properties that define the item ingame that are rarely/never updated (metadata).
    ...
    @staticmethod
    def sqlite_columns(Item) -> Tuple[str, ...]:
    ...
    def sql_select() -> str:
        ...
        return f"""SELECT ({", ".join(Item.sqlite_columns())}) FROM "{ITEM_TABLE}" """
