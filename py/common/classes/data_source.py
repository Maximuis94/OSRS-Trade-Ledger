...
class DataSource:
    ...
    db_path : str
        Path to the timeseries database the data is drawn from
    ...
    db_path: str = field(default=str(gp.f_db_timeseries), compare=False)
    ...
    def connect(self) -> sqlite3.Connection:
        ...
        return sqlite3.connect(database=f"file:{self.db_path}?mode=ro", uri=True)
    ...
    def fetch_data(self, item_id: int, t0: int, t1: int = int(time.time())) -> List[any]:
        ...
        conn = self.connect()
        return conn.execute(self._sql_fetch.replace('_____', f'{item_id:0>5}'), (t0, t1)).fetchall()
