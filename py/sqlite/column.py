...
def parse_columns(c: sqlite3.Cursor, table: str) -> List[Column]:
    ...
    # print(sql[pk_idx:])
    # print(sql[pk_idx:sql.index(")", pk_idx)].split(")")[0])
    pks = [pk.strip(""""'""") for pk in sql[pk_idx:sql.index(")", pk_idx)].split(")")[0].split(",")]
    # pks = [pk.strip(""""'""") for pk in sql.split("PRIMARY KEY(")[-1].split(")")[0].split(",")]
    # print(pks)
    for col in sql[sql.index("(") + 1:].split(","):
        ...
        # print(Column(id=len(output), name=name, table=table, sql_dtype=dtype, **kws)._asdict())
